...
namespace glfw
{
  ...
  IGL_INLINE void Viewer::simplify_mesh(int num_of_faces_to_delete) {
      ...
      printf("this will be activated later on \n");
    /* 
      int num_of_collapsed_edges = 0; 
      
      for (int i = 0; i < num_of_faces_to_delete; i++) {
          // this function will be written in the future 
          if (collapse_edge()) {

              num_of_collapsed_edges++;
      ...
      if (num_of_collapsed_edges > 0) {
          printf("succesfully collapsed # %d of edges", num_of_collapsed_edges);
          data().clear();
...
*/
  ...
  IGL_INLINE bool Viewer::collapse_edge() { return true;  }
